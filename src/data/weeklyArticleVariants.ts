...
/** 밝은 느낌 + 기사 핵심 키워드별 Unsplash photo ID (모든 기사 주제 커버) */
const KEYWORD_IMAGES: Array<{ when: RegExp; id: string }> = [
  // 정치
  { when: /(투표|선거|민주주의|공약|후보|유권자)/, id: '1540910419892-4a36d2c3266c' },
  { when: /(국회|입법|정국|여야|법안)/, id: '1540910419892-4a36d2c3266c' },
  { when: /(태극기|국기|3·1|독립|독립운동|애국가)/, id: '1540914122471-2b16eae459de' },
  { when: /(남북|한반도|평화|국방|병역|군)/, id: '1524661135-423995f22d0b' },
  { when: /(역사 인식|역사 교과서|역사 논의)/, id: '1540914122471-2b16eae459de' },
  // 경제
  { when: /(전기요금|전기료|가스 요금|에너지 가격|난방)/, id: '1473341304177-3d6342c5b8c6' },
  { when: /(돈|용돈|저축|월급|봉급|세뱃돈|물건을 사요)/, id: '1553729459-0feda8a5386f' },
  { when: /(경제|물가|인플레이션|공급망|수출|경상수지)/, id: '1579621970563-ebec7560ff3e' },
  { when: /(환율|원화|달러|엔|환전)/, id: '1611972616597-649dce757427' },
  { when: /(금리|전세|매매|주택)/, id: '1560518883-ce09059e6725' },
  { when: /(구직|취업|일자리|최저임금|아르바이트)/, id: '1521737715017-d6805ec7ed54' },
  { when: /(바이오|헬스케어|신약|진단)/, id: '1576092764391-3510ef7972a3' },
  { when: /(반도체|수출 규제|미중)/, id: '1568967729541-096375bef6e8' },
  // 사회
  { when: /(귀성|교통|고속도로|KTX|버스|수송)/, id: '1544620347-c4fe4d3a4d59' },
  { when: /(친구|도와|반에서|다문화|공동체|서로)/, id: '1529156069898-49953e39b3ac' },
  { when: /(가족|할머니|이웃|명절)/, id: '1529156069898-49953e39b3ac' },
  { when: /(저출산|고령화|노인|시니어|실버)/, id: '1600887012170-422167697c84' },
  { when: /(학교폭력|상담|법적 대응|갈등)/, id: '1523050854058-24b38136d167' },
  { when: /(촛불|집회|시민 참여)/, id: '1517245385427-6c4a9e64a108' },
  { when: /(교통 안전|안전사고|재난|지진|화재)/, id: '1544620347-c4fe4d3a4d59' },
  // 문화
  { when: /(설날|명절|윷놀이|제기차기|비석치기|세배)/, id: '1528360983277-518d751faf61' },
  { when: /(발렌타인|초콜릿|사랑 나눠요)/, id: '1513546483692-cb967e75eb7a' },
  { when: /(한복|전통 놀이|판소리|전통음악)/, id: '1528360983277-518d751faf61' },
  { when: /(K팝|한류|드라마|웹툰|웹소설|K콘텐츠|콘텐츠)/, id: '1493225457124-ccbcddeba207' },
  { when: /(봄꽃|벚꽃|진달래|꽃이 피어요|등산|캠핑|아웃도어)/, id: '1523241287236-6470d976904f' },
  { when: /(OTT|넷플릭스|웨이브|스트리밍|미디어 산업)/, id: '1611162616475-46b6352e64ce' },
  { when: /(저작권|알고리즘|뉴스 추천|SNS)/, id: '1611162616475-46b6352e64ce' },
  // 과학
  { when: /(태양|해가|달|위상|삭|망)/, id: '1507003211169-0a1dd7228f2d' },
  { when: /(우주|로켓|탐사|누리호|스페이스X|스타링크)/, id: '1451187580459-43490279c0e6' },
  { when: /(AI|인공지능|챗봇|딥페이크|가짜 뉴스)/, id: '1676299082083-2dc7f3d440b4' },
  { when: /(CRISPR|유전자|양자컴퓨터)/, id: '1676299082083-2dc7f3d440b4' },
  { when: /(전기차|배터리)/, id: '1593941707882-6fc9552d0e76' },
  { when: /(수소|청색수소)/, id: '1559757148-5c4a2c950330' },
  { when: /(눈이 오는|한파|겨울|기후변화)/, id: '1491002054626-602b485f489f' },
  { when: /(봄이 오면|꽃이 피어요|계절|식물)/, id: '1490750967868-88aa4986a51c' },
  { when: /(비눗방울|과학실험)/, id: '1581092160569-d6d2e7635a44' },
  // 교육
  { when: /(개학|새 학기|교실|학교|방학)/, id: '1523050854058-24b38136d167' },
  { when: /(입시|수시|정시|수능|시험|수험생|등록금|장학)/, id: '1434030216841-604d0b114f72' },
  { when: /(디지털 교육|태블릿|학습부진|멘토링|학점제|진로)/, id: '1503676260728-1c00da094a0b' },
  { when: /(교육정책|대입|사교육)/, id: '1434030216841-604d0b114f72' },
  { when: /(인권|교권|학생 인권)/, id: '1523050854058-24b38136d167' },
  // 환경
  { when: /(미세먼지|대기질|마스크)/, id: '1584305567982-91ec7c132d2f' },
  { when: /(재생에너지|태양광|풍력|넷제로|기후 위기)/, id: '1509391366360-2e77ee52d5d2' },
  { when: /(플라스틱|바다|재활용|분리수거|탄소)/, id: '1532992742220-6ac5bc4d8b62' },
  { when: /(산불|예방)/, id: '1534368959575-1927319b2c05' },
  { when: /(난방|탄소 배출|환경 오염)/, id: '1542601906990-b4d3fb778b09' },
  // 국제
  { when: /(해외 여행|동남아|일본)/, id: '1524661135-423995f22d0b' },
  { when: /(UN|세계 평화|국제기구)/, id: '1524661135-423995f22d0b' },
  { when: /(동아시아|한미일|정상회담|외교)/, id: '1524661135-423995f22d0b' },
  { when: /(우크라이나|재건|FTA|G20|국제회의)/, id: '1473341304177-3d6342c5b8c6' },
  { when: /(유럽|에너지 위기|가스|전력)/, id: '1473341304177-3d6342c5b8c6' },
  // 법·건강·스포츠
  { when: /(명절 사고|법적 책임|교통·음주)/, id: '1589829545856-d10d557cf95f' },
  { when: /(소년법|아동권리|보호법|개인정보|표현|혐오)/, id: '1589829545856-d10d557cf95f' },
  { when: /(설 음식|소화기|과식|명절 건강)/, id: '1544367567-0f2fcb009e0b' },
  { when: /(정신건강|우울|불안|수면|운동 부족|알레르기|꽃가루)/, id: '1544367567-0f2fcb009e0b' },
  { when: /(동계|스키|스케이팅)/, id: '1551522435-a89afa12578c' },
  { when: /(야구|KBO|개막)/, id: '1574629810360-7efbe195a735' },
  { when: /(축구|K리그|유럽 리그)/, id: '1542751371-adc38448a05e' },
  { when: /(농구|배구|KBL|V리그|플레이오프)/, id: '1546519638-af2f498e742f' },
  { when: /(e스포츠|게임)/, id: '1542751371-adc38448a05e' },
  { when: /(문화예술|공영|방송)/, id: '1511671782779-c97d3d27a1d4' },
];

const SECTION_IMAGES: Record<string, string> = {
  정치: U('1540910419892-4a36d2c3266c'),
  경제: U('1553729459-0feda8a5386f'),
  사회: U('1529156069898-49953e39b3ac'),
  문화: U('1528360983277-518d751faf61'),
  과학: U('1451187580459-43490279c0e6'),
  교육: U('1503676260728-1c00da094a0b'),
  환경: U('1542601906990-b4d3fb778b09'),
  국제: U('1524661135-423995f22d0b'),
  미디어: U('1611162616475-46b6352e64ce'),
  법: U('1589829545856-d10d557cf95f'),
  건강: U('1544367567-0f2fcb009e0b'),
  스포츠: U('1542751371-adc38448a05e'),
};
...
function getImageUrlForArticle(params: {
  ...
  for (const rule of KEYWORD_IMAGES) {
    if (rule.when.test(text)) return U(rule.id);
  }
  return SECTION_IMAGES[params.section] ?? U('1540910419892-4a36d2c3266c');
}
